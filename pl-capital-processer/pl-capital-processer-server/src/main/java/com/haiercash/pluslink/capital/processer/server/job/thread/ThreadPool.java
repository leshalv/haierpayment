...
public class ThreadPool extends ThreadGroup {
    ...
    protected LinkedList<IWorker> workQueue = new LinkedList<>();      //工作队列

    protected LinkedList<IWorker> workingQueue = new LinkedList<>();      //工作队列

    protected static int threadPoolId = 0;  //线程池的id

    protected LinkedList<WorkThread> runningThread = new LinkedList<>();
    ...
    public synchronized boolean isWaiting(String id) {
    ...
    public synchronized IWorker getRunningWorker(String id) {
        for (IWorker worker : this.workingQueue) {
            if (worker.getId().equals(id)) {
                return worker;
            }
    ...
    public synchronized IWorker getWaitingWorker(String id) {
        for (IWorker worker : this.workQueue) {
            if (worker.getId().equals(id)) {
                return worker;
            }
     ...
     * @return
     */
    public synchronized IWorker getWorker(String id) {
        for (IWorker worker : this.workQueue) {
            if (worker.getId().equals(id)) {
                return worker;
            }
