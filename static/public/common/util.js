define('util', ['vue', 'jquery', 'Const', 'text!template/tags.html', 'moment', 'toastr', 'json'], function(vue, $, Const, template, moment, toastr) {
    ...
    return {
        ...
        week: function (pattern, date) {
            ...
            return moment(date, pattern).weekday();
        },
        /******************************************* 数据处理 *********************************************************/
...
//            this.scroll($element);
        ...
        redirect: function(url, type) {
            if (!type && this.type(url) === 'string') {
                ...
                Const.init.path = url;
                if ((param.popstate === undefined || param.popstate !== false)) {
                    url = Const.route.baseLocation + Const.route.versionLocation + (url && url !== '/' ? '/#!' + url : '/');
                    if (param.ignore) {
                        if (param.back === false) {
                        ...
                        if (param.columns) {
                            if (this.type(param.columns) === 'array') {
                                for (var i=0; i<param.columns.length; i++) {
                                    param.vm.columns.push(param.vm.initColumn(param.columns[i]));
                                    param.vm.initEntity[param.columns[i].name] = null;
                                }
                            } else {
                                // json
                                for (var p in param.columns) {
                                    param.vm.columns.push(param.vm.initColumn({
                                        name: p,
                                        head: p,
                                        config: {
        ...
        initDefault: function(vm) {
            ...
            if (this.isEmpty(vm.value) && vm.entity && vm.name) {
        ...
        */
                ...
                for (var i=0; i<vm.initParamList.length; i++) {
                    ...
                    if (initParam.from) {
                        if (initParam.duplex) {
                            ...
                            initParam.value = vm.entity[initParam.from];
                        }
        ...
        initSelectOptions: function(vm, initOptions, type, trigger) {
            ...
            if (trigger && vm.entity && vm.name && vm.entity[vm.name]) {
