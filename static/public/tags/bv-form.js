    ...
    vue.component('bv-form', {
        ...
        created: function () {
            ...
            if (this.editType === 'insert') {
                for (var i=0; i<this.columns.length; i++) {
                    if (!util.isEmpty(this.columns[i].name)) {
                        this.initEntity[this.columns[i].name] = null;
                    }
                }
        ...
        methods: {
            ...
            isColumnCreate: function(column) {
                if (!column || column.edit.type === 'ignore' || column.destroy === true || column.create === false) {
                    this.entity[column.name] = null;
                    return false;
                }
                var destroy;
            ...
            click: function(event, operate) {
                if (operate) {
                    if (operate.preset) {
                                ...
                                if (this.isCustomSave) {
                                    ...
                                    if (this.checks.length > 0) {
                                        for (var i=0; i<this.checks.length; i++) {
                                            this.checks[i].value = this.entity[this.check[i].name];
                                        }
                        ...
                        operate.click.call(null, $(event.target), this.editType, this.entity, this.extraParams);
                    }
                }
            },
